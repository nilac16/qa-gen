                                   ...
                                   const wchar_t        *title)
{
    ...
    hr = PDLG_METHOD(pdlg->pd, SetTitle)(pdlg->pd, title);
    if (FAILED(hr)) {
        qagen_error_raise(QAGEN_ERR_HRESULT, &hr, failmsg);
    }
    return FAILED(hr);
...
static int qagen_progdlg_set_cancelmsg(struct qagen_progdlg *pdlg)
{
    ...
    hr = PDLG_METHOD(pdlg->pd, SetCancelMsg)(pdlg->pd, L"Stopping...", NULL);
    if (FAILED(hr)) {
        qagen_error_raise(QAGEN_ERR_HRESULT, &hr, failmsg);
    }
    return FAILED(hr);
...
static int qagen_progdlg_create(struct qagen_progdlg *pdlg)
{
    ...
    hr = PDLG_METHOD(pdlg->pd, StartProgressDialog)(pdlg->pd,
                                                    NULL,
                                                    NULL,
                                                    PROGDLG_AUTOTIME,
                                                    NULL);
...
void qagen_progdlg_destroy(struct qagen_progdlg *pdlg)
{
    ...
    hr = PDLG_METHOD(pdlg->pd, StopProgressDialog)(pdlg->pd);
    if (FAILED(hr)) {
